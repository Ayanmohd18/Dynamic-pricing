...
st.markdown("### Phase 2: Real-Time Inference & Competitor Monitoring")
...
with col2:
    ...
    # Simulating the derived features (sin/cos etc) for the dashboard
    payload = {
        "sku_id": sku_id,
        "price": base_price,
        "freight_value": 20.0,
    ...
    try:
        # Note: Assumes FastAPI is running locally on port 8000
        ...
        if response.status_code == 200:
            ...
            st.error("API connection failed. Make sure the FastAPI server is running on localhost:8000.")
            
    except Exception as e:
        st.info("💡 Start the FastAPI server (src/api/main.py) to see real-time price updates.")

st.divider()
st.markdown("#### Logic Trace (Phase 2)")
st.code("""
